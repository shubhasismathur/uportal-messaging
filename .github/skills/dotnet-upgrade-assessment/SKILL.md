...
Call the MCP tool `get_state` with the workspace path to initialize the workflow state.
If the tool accepts a `path` parameter, pass `{workspace-path}`. If the call fails with an unexpected argument error, retry without the path parameter (for backward compatibility with older MCP servers).
