...
- **Rule 6 — Never downgrade the current JDK version**: The resolved target JDK MUST never be lower than the project's current JDK. If a selected task type's default JDK is lower than the current version, keep the current version instead of downgrading. This holds even when the current version is a non-LTS release past end of support (e.g., Java 22): never downgrade it to a lower LTS (e.g., Java 21).