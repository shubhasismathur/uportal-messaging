...
def load_config(config_path):
    ...
    if not isinstance(java, dict):
        if security is not None:
            # Top-level security present but no java section: run security domain only.
            log("Config has no 'java' section but has top-level security; running security only.")
            return {
                "assessmentDomains": ["security"],
                "analysisCoverage": root_coverage or "issue-only",
                "targetRuntime": None,
                "targetComputeServices": [],
                "targetOS": [],
                "enableContainerization": False,
                "minimumCveSeverity": root_min_cve or DEFAULT_MINIMUM_CVE_SEVERITY,
            }
        else:
            # No java section, no security: fall back to full default config.
    ...
    # Has java section: build config from it.
    cfg = {
    ...
    # Inject security domain when top-level security is enabled.
    if security is not None and "security" not in [d.lower() for d in cfg["assessmentDomains"]]:
        cfg["assessmentDomains"].append("security")
    return cfg
