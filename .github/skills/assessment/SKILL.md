...
2. **Run Assessment**:
   ...
   - **For JavaScript/TypeScript projects**: Install and run npm-check-updates
     - Resolve the machine's global npmrc first: `npm config get globalconfig`
     - Install: `npm install -g npm-check-updates@19.6.3 --prefix {tool-install-dir} --globalconfig {global-npmrc}`
       - `--prefix` on its own makes npm look for its global config at `{tool-install-dir}/etc/npmrc`, silently discarding the machine's registry, proxy and cafile settings and falling back to registry.npmjs.org. Pinning `--globalconfig` keeps them. Omit that flag only if the resolved path does not exist.
